...
class DrunkardWalk:
    ...
    def plot_path(self, show: bool = True, save_path: Optional[str] = None) -> None:
        ...
        if self.dimensions == 1:
            ...
            max_range = np.array([path_array[:,0].max()-path_array[:,0].min(),
                                  path_array[:,1].max()-path_array[:,1].min(),
                                  path_array[:,2].max()-path_array[:,2].min()]).max() / 2.0
            mid_x = (path_array[:,0].max()+path_array[:,0].min()) * 0.5
            mid_y = (path_array[:,1].max()+path_array[:,1].min()) * 0.5
            mid_z = (path_array[:,2].max()+path_array[:,2].min()) * 0.5
...
def create_animation(walker_path: PathArray, dimensions: int, step_type: str, fps: int = 30, show_animation: bool = True, save_path: Optional[str] = None, blit: bool = True) -> Optional[animation.FuncAnimation]:
    ...
    blit : bool
        Whether to use blitting for optimization (may cause issues on some backends)
    ...
    title = f'{dimensions}D Random Walk ({step_type}) - Step {{frame}}/{num_frames-1}' # Dynamic title

    if dimensions == 1:
        ax = fig.add_subplot(111)
        line, = ax.plot([], [], '-o', alpha=0.7, markersize=3) # Path line
        point, = ax.plot([], [], 'ro', markersize=8) # Current position marker
        time_text = ax.text(0.02, 0.95, '', transform=ax.transAxes) # Step counter text
        ...
        def init():
            ...
            ax.grid(True)
            time_text.set_text('')
            ax.set_title(title.format(frame=0)) # Set initial title
        ...
        def update(frame):
            ...
            time_text.set_text(f'Step: {frame}/{num_frames-1}')
            # Note: Updating title in update can be slow, consider static title or text object
            # ax.set_title(title.format(frame=frame)) # Option 1: Update title (slower)
            return line, point, time_text

    elif dimensions == 2:
        ax = fig.add_subplot(111)
        line, = ax.plot([], [], '-', alpha=0.7, label='Path') # Path line
        point, = ax.plot([], [], 'ro', markersize=8, label='Current Position') # Current position
        ...
        def init():
            ...
            time_text.set_text('')
            ax.set_title(title.format(frame=0))
        ...
        def update(frame):
            ...
            time_text.set_text(f'Step: {frame}/{num_frames-1}')
            # ax.set_title(title.format(frame=frame)) # Option 1: Update title (slower)
            return line, point, start, time_text # Return start marker as well if it needs redraw (though static here)

    elif dimensions == 3:
        ax = fig.add_subplot(111, projection='3d')
        line, = ax.plot([], [], [], '-', alpha=0.7, label='Path') # Path line
        # Use scatter for current point for better 3D visibility
        point = ax.scatter([], [], [], color='r', s=50, label='Current Position', depthshade=True)
        start = ax.scatter([], [], [], color='g', s=100, label='Start', depthshade=True)
        time_text = ax.text2D(0.02, 0.95, '', transform=ax.transAxes) # Step counter text (use text2D for 3D plots)


        # Determine fixed axes limits for stable view
        x_min, x_max = walker_path[:, 0].min(), walker_path[:, 0].max()
        y_min, y_max = walker_path[:, 1].min(), walker_path[:, 1].max()
        z_min, z_max = walker_path[:, 2].min(), walker_path[:, 2].max()
        # Create cubic bounding box based on largest range
        max_range = np.array([x_max-x_min, y_max-y_min, z_max-z_min]).max()
        mid_x = (x_max + x_min) * 0.5
        mid_y = (y_max + y_min) * 0.5
        mid_z = (z_max + z_min) * 0.5
        ax.set_xlim(mid_x - max_range/2 * 1.1, mid_x + max_range/2 * 1.1) # Add 10% margin
        ax.set_ylim(mid_y - max_range/2 * 1.1, mid_y + max_range/2 * 1.1)
        ax.set_zlim(mid_z - max_range/2 * 1.1, mid_z + max_range/2 * 1.1)


        def init():
            line.set_data([], [])
            line.set_3d_properties([])
            # For scatter, we update _offsets3d in update, init might not be needed or set to empty
            point._offsets3d = ([], [], [])
            ...
            time_text.set_text('')
            ax.set_title(title.format(frame=0))
            # Note: Returning scatter artists in init/update for blitting can be complex/problematic.
            # Blitting might need to be False for reliable 3D scatter animation.
            return line, point, start, time_text
        ...
        def update(frame):
            ...
            time_text.set_text(f'Step: {frame}/{num_frames-1}')
            # ax.set_title(title.format(frame=frame)) # Option 1: Update title (slower)
            # If blit=True, MUST return all animated artists
            return line, point, start, time_text
    ...
    # Interval is delay between frames in ms. interval = 1000 / fps
    try:
        # Try disabling blit for 3D scatter plots if issues arise
        ...
        if save_path:
            try:
                ...
                print("Try running with --no-blit if you encounter backend issues.\n")
...
def plot_msd(steps: np.ndarray, msd: np.ndarray, step_type: str, dimensions: int, save_path: Optional[str] = None, show: bool = True) -> None:
    ...
    ax.loglog(steps, steps, 'k--', alpha=0.7, label=r'MSD $\propto t$ (Normal Diffusion)')

    # Super-diffusion example (relevant for Levy flights): MSD ~ t^alpha with alpha > 1
    # For Cauchy (alpha=1 stable), MSD behavior can be complex, often faster than t but potentially not a simple power law.
    # For general Levy stable flights with stability 0 < alpha < 2, expect MSD ~ t^(2/alpha) if finite variance exists,
    # but variance is infinite for alpha<=2. Ballistic is t^2.
    # Let's plot t^1.5 and t^2 as references for super-diffusion/ballistic motion.
    if step_type == 'levy' or np.any(msd > steps * 1.5): # Show super-diffusion lines if relevant
         # Avoid potential RuntimeWarning for power overflow with large steps
         valid_steps = steps[steps > 0]
         power_1_5 = np.power(valid_steps.astype(np.float64), 1.5) * (msd[1]/steps[1] if len(steps)>1 else 1.0) # Scale roughly
         ax.loglog(valid_steps, power_1_5, 'r--', alpha=0.7, label=r'MSD $\propto t^{1.5}$')
        # power_2_0 = np.power(valid_steps.astype(np.float64), 2.0) * (msd[1]/steps[1] if len(steps)>1 else 1.0) # Ballistic
        # ax.loglog(valid_steps, power_2_0, 'g--', alpha=0.7, label=r'MSD $\propto t^2$ (Ballistic)')
...
def main():
    ...
    if args.walkers == 1:
        ...
        if args.analyze_msd:
             ...
             msd_save_path = None
             if args.save_plot:
                 # Generate a sensible filename for the MSD plot if saving the main plot
                 base, ext = os.path.splitext(args.save_plot)
                 msd_save_path = f"{base}_msd{ext}"

             plot_msd(msd_steps, msd_values, args.step_type, args.dimensions, save_path=msd_save_path, show=True) # Always show MSD for now
        ...
        print(f"Mean displacement (from {np.sum(~np.isnan(displacements))} valid walkers): {mean_disp:.3f}")
        # Theoretical expectation for simple random walk (uniform/gaussian steps) is sqrt(steps * dimensions_factor)
        # For uniform step = +/- 1 along one axis, expected MSD = t. So RMS displacement = sqrt(t).
        # For gaussian N(0,1) steps in d-dims, expected MSD = d*t. So RMS displacement = sqrt(d*t).
        ...
        # No simple theoretical value for Levy mean displacement (often infinite variance)
